...
const MembershipSelection = () => {
  ...
  const handleConfirm = async () => {
    ...
    const planDetails = membershipPlans.find(plan => plan.id === selectedPlan);
    setSelectedMembershipDetails({
      tier: selectedPlan,
      name: planDetails.name,
      price: planDetails.price
    });
    
    // Show payment gateway instead of submitting directly
    setShowPayment(true);
  };
  
  const handlePaymentSuccess = async () => {
    try {
      console.log("Payment successful, submitting membership selection");
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
      ...
      {showPayment && selectedMembershipDetails && (
        <PaymentGateway 
          membershipDetails={selectedMembershipDetails}
