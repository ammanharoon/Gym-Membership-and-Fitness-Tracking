...
router.get("/fitness/stats", async (req, res) => {
  try {
    const userId = req.user.id;
...
// Add a weight entry
router.post("/fitness/weight", async (req, res) => {
  try {
    const userId = req.user.id;
...
router.post("/fitness/body-fat", async (req, res) => {
  try {
    const userId = req.user.id;
    const { value, date } = req.body;
...
router.post("/fitness/water-intake", async (req, res) => {
  try {
    const userId = req.user.id;
    const { value, date } = req.body;
...
router.post("/fitness/workout-logs", async (req, res) => {
  try {
    const userId = req.user.id;
...
router.delete("/fitness/workout-logs/:id", async (req, res) => {
  try {
    const userId = req.user.id;
...
router.patch("/fitness/goals/:id/progress", async (req, res) => {
  try {
    const userId = req.user.id;
    const goalId = req.params.id;
